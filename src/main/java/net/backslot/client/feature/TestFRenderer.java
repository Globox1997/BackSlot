...
public class TestFRenderer<T extends LivingEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {

    public TestFRenderer(FeatureRendererContext<T, M> context) {
        super(context);
    }
    ...
    @Override
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress,
            float headYaw, float headPitch) {
        PlayerEntity player = (PlayerEntity) entity;
        ItemStack backSlotStack = player.getInventory().getStack(41);
        if (entity instanceof AbstractClientPlayerEntity && !backSlotStack.isEmpty()) {
            matrixStack.push();
            ...
            if (!this.isSpecialModelItem(backSloItem, loader)) {
                ...
                MinecraftClient.getInstance().getHeldItemRenderer().renderItem(entity, backSlotStack, ModelTransformation.Mode.GROUND, false, matrixStack, vertexConsumerProvider, i);
            } else {
                ...
                MinecraftClient.getInstance().getHeldItemRenderer().renderItem(entity, backSlotStack, ModelTransformation.Mode.THIRD_PERSON_RIGHT_HAND, false, matrixStack, vertexConsumerProvider, i);
            }
            matrixStack.pop();
        }
    }
